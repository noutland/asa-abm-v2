...
org <- create_organization(
  n_agents = 250,
  identity_categories = c("Tech", "Sales", "Ops")
)
```
...
applicants <- create_applicant_pool(
  n_applicants = 200,
  identity_categories = c("Eng", "Prod", "Sales")
)
```
...
**Usage**
```r
calculate_applicant_attraction(applicants, org)
```
...
applicants <- calculate_applicant_attraction(applicants, org)
...
**Usage**
```r
update_satisfaction_vectorized(org, interactions, window_size = 10)
```
